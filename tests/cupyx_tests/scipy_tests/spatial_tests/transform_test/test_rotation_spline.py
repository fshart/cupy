...
from cupyx.scipy.spatial.transform import Rotation, RotationSpline
from cupyx.scipy.spatial.transform._rotation_spline import (
...
def test_angular_rate_to_rotvec_conversions():
    ...
    I_stack = cp.empty((4, 3, 3))
    I_stack[:] = cp.eye(3)
    assert_allclose(cp.matmul(A, A_inv), I_stack, atol=1e-15)
...
def test_create_block_3_diagonal_matrix():
    cp.random.seed(0)
    A = cp.empty((4, 3, 3))
    A[:] = cp.arange(1, 5)[:, None, None]

    B = cp.empty((4, 3, 3))
    B[:] = -cp.arange(1, 5)[:, None, None]
    d = 10 * cp.arange(10, 15)

    banded = _create_block_3_diagonal_matrix(A, B, d)

    # Convert the banded matrix to the full matrix.
    k, l = list(zip(*product(cp.arange(banded.shape[0]),
                             cp.arange(banded.shape[1]))))
    k = cp.asarray(k)
    l = cp.asarray(l)
...
def test_spline_2_rotations():
    ...
    times_check = cp.array([-1, 5, 12])
    dt = times_check - times[0]
    rv_ref = rate * dt[:, None]

    assert_allclose(spline(times_check).as_rotvec(), rv_ref)
    assert_allclose(spline(times_check, 1), cp.resize(rate, (3, 3)))
    assert_allclose(spline(times_check, 2), 0, atol=1e-16)


def test_constant_attitude():
    times = cp.arange(10)
    rotations = Rotation.from_rotvec(cp.ones((10, 3)))
    spline = RotationSpline(times, rotations)

    times_check = cp.linspace(-1, 11)
...
def test_error_handling():
    ...
    raises(ValueError, s, cp.arange(10).reshape(5, 2))
